...
//*.jsp 와 같은 url 형식인 모든 요청에 대해 처리될 필터이다. 목적: session 검사하기
@SuppressWarnings("serial")
@WebFilter(urlPatterns = {"*.jsp"})	//web.xml에서 filter로 대신 사용할 수 있다.
public class SessionFilter extends HttpFilter implements Filter {
	...
	public void doFilter(ServletRequest req, ServletResponse resp, FilterChain chain) 
		...
		//메소드의 인자로 전달된 것이 HttpXXXX 의 부모 인터페이스이므로 캐스팅이 필요하다.
		...
		if(logins.contains(request.getServletPath()) && user == null){
			//요청 경로가 logins 중에 하나이고 로그인이 안되었다면(user == null) 접근 못함
			//	contextPath 로 강제 리다이렉트			
			response.sendRedirect(request.getContextPath());
			return;		//필터 실행 종료
		}
		...
		//예시 http://localhost:8081/jspBoard/updateAction.jsp?idx=46&content=ㅋㅋㅋ&title=테스트2&page=1  가 실행되지 않도록 함
		...
		//다음 필터가 있으면 요청 또는 응답을 전달하고, 없으면 요청 또는 응답처리 한다.
